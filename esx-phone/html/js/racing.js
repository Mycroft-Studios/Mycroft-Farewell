...
$(document).on('click', '.racing-race', function(e){
    ...
    var IsRacer = IsInRace(Phone.Data.PlayerData.identifier, RaceData.RaceData.Racers)
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "The race already started..", "#1DA1F2");
    }
});
...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            ...
            var InRace = IsInRace(Phone.Data.PlayerData.identifier, race.RaceData.Racers);
            var Creator = IsCreator(Phone.Data.PlayerData.identifier, race);
...
$(document).on('click', '#join-race', function(e){
    ...
    $.post('https://esx-phone/IsInRace', JSON.stringify({}), function(IsInRace){
        if (!IsInRace) {
            $.post('https://esx-phone/RaceDistanceCheck', JSON.stringify({
                RaceId: Data.RaceId,
                Joined: true,
            }), function(InDistance){
                if (InDistance) {
                    $.post('https://esx-phone/IsBusyCheck', JSON.stringify({
                        check: "editor"
                    }), function(IsBusy){
                        if (!IsBusy) {
                            $.post('https://esx-phone/JoinRace', JSON.stringify({
                                RaceData: Data,
                            }));
                            ...
                            Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You're in a editor..", "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You're already in a race..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#quit-race', function(e){
    ...
    $.post('https://esx-phone/LeaveRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '#start-race', function(e){
    ...
    $.post('https://esx-phone/StartRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('https://esx-phone/GetTrackData', JSON.stringify({
        RaceId: $(this).attr('id')
    }), function(TrackData){
        if ((TrackData.CreatorData.lastname).length > 8) {
            TrackData.CreatorData.lastname = TrackData.CreatorData.lastname.substring(0, 8);
        }
        var CreatorTag = TrackData.CreatorData.firstname.charAt(0).toUpperCase() + ". " + TrackData.CreatorData.lastname;
...
$(document).on('click', '#setup-race', function(e){
    ...
    $.post('https://esx-phone/GetRaces', JSON.stringify({}), function(Races){
...
$(document).on('click', '#create-race', function(e){
    e.preventDefault();
    $.post('https://esx-phone/IsAuthorizedToCreateRaces', JSON.stringify({}), function(data){
            ...
            if (!data.IsBusy) {
                $.post('https://esx-phone/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You're in a race..", "#1DA1F2");
                    }
                });
            } else {
                Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You're already setting up a track..", "#1DA1F2");
            }
        } else {
            Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You don't have rights to make Race Tracks..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        ...
        $.post('https://esx-phone/IsAuthorizedToCreateRaces', JSON.stringify({
            TrackName: TrackName
        }), function(data){
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    $.post('https://esx-phone/StartTrackEditor', JSON.stringify({
                        TrackName: TrackName
                    }));
                    ...
                    Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "This name is not available..", "#1DA1F2");
                }
            } else {
                Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You don't have any rights to create Race Tracks..", "#1DA1F2");
            }
        });
    } else {
        Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You have to enter a track name..", "#1DA1F2");
    }
});
...
$(document).on('click', '#setup-race-accept', function(e){
    ...
    $.post('https://esx-phone/HasCreatedRace', JSON.stringify({}), function(HasCreatedRace){
        if (!HasCreatedRace) {
            $.post('https://esx-phone/RaceDistanceCheck', JSON.stringify({
                RaceId: track,
                Joined: false,
                ...
                if (InDistance) {
                    if (track !== undefined || track !== null) {
                        if (laps !== "") {
                            $.post('https://esx-phone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    $.post('https://esx-phone/SetupRace', JSON.stringify({
                                        RaceId: track,
                                        AmountOfLaps: laps,
                                    }))
                                    $(".racing-overview").animate({
                                    ...
                                    Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "There can't be any ..", "#1DA1F2");
                                }
                            });
                        } else {
                            Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Fill in an amount of laps..", "#1DA1F2");
                        }
                    } else {
                        Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You haven't selected a track..", "#1DA1F2");
                    }
                }
            })
        } else {
            Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You already have a race active..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '.racing-leaderboard-item', function(e){
    ...
    $.each(Data.LastLeaderboard, function(i, leaderboard){
        ...
        var elem = '<div class="rowsab"><div class="name">'
            + ((leaderboard.Holder[0]).charAt(0)).toUpperCase() + 
            '. ' + lastname + '</div>' +
            '<div class="time">'+bestroundtime+'</div><div class="score">'+ place +'</div> </div>';
...
$(document).on('click', '#leaderboards-race', function(e){
    ...
    $.post('https://esx-phone/GetRacingLeaderboards', JSON.stringify({}), function(Races){
        if (Races !== null) {
