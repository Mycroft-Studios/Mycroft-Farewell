...
$(document).on('click', '.mail', function(e){
    ...
    Phone.Functions.SetupMail(MailData);
...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('https://esx-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('https://esx-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(Phone.Data.PlayerData.name+ "@anotherlevelrp.com");
...
Phone.Functions.SetupMail = function(MailData) {
...
$(document).on('click','#new-advert-photo',function(e){
    e.preventDefault();
    $.post('https://esx-phone/TakePhoto',function(url){
        if(url){
...
Phone.Functions.RefreshAdverts = function(Adverts) {
    $("#advert-header-name").html("@"+Phone.Data.PlayerData.name+" | "+Phone.Data.PlayerData.phone_number);
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $.each(Adverts, function(i, advert){
            ...
            if (advert.number === Phone.Data.PlayerData.phone_number){
...
$(document).on('click','#adv-delete',function(e){
    e.preventDefault();
    $.post('https://esx-phone/DeleteAdvert', function(){
        setTimeout(function(){
            Phone.Notifications.Add("fas fa-ad", "Advertisement", "The ad was deleted", "#ff8f1a", 2000);
        },400)
    });
})
